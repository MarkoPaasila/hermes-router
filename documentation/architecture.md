...
provider in a pool, handling sticky key selection, failover, and format translation transparently.
...
4. **Order the catalog** — every configured `(provider, model)` is scored 1–5 for capability;
   the router prefers the *cheapest* candidate that can handle the request, skips unhealthy
   ones, and promotes a session-sticky match to the front when present.
5. **Try and fail over** — it sends to the first candidate, preferring the sticky key when
   set; on rate-limit or error it cascades to the next catalog entry, clearing stickiness when
   leaving the sticky `(provider, model)`.
...
(TBF): it learns caps from response headers and 429s, **force-admits** when estimates look
...
support function calling (detected at startup). Optional **fast routing**
(`FAST_ROUTE_THRESHOLD`) sends short requests to low-latency providers first.

**Session-sticky routing.** When the client sends a session id (header `X-Hermes-Session-Id` or
`X-Chat-ID`, body `user`, or `metadata.session_id` / `sessionId` — first non-empty wins; the
router never invents one), the router remembers the winning `(provider, model, key)` and reuses
...
| `GET /v1/status` | proxy key | Per-provider health, latency, keys, sticky-key mode, cache |
