...
> *front-end*; it talks to a router, it isn't the proxy itself.
...
- **Status bar says the proxy is down / "unreachable"** — the extension can't reach `baseUrl`.
  Confirm the proxy is running (`curl <baseUrl>/health`) and that `hermesRouter.baseUrl` points
...
- **"Restart / Doctor / Update" says `hr` isn't on your PATH (or `spawn hr ENOENT`)** — those
  ...
  **Windows** host or inside a container that doesn't bundle it:
  - **Docker:** set `hermesRouter.dockerContainer` to your container's name so these commands run
    via `docker exec`/`docker restart` instead (see "Using the proxy in Docker" above) — or just
  ...
  - **Windows without Docker:** run the proxy under **WSL2**, where `hr` works.
...
See also: [deployment.md](deployment.md) (run the proxy locally, in Docker, or on a Space) and
